...
def PlotIntensity(ks,ws,inten,angulo,fac=1.0,Imax='none',units='au',fout=''):
   ...
   if units == 'au':
      ...
      ax.set_xlabel(r"$k_\parallel \, $ (au)",fontsize=20)
      ax.set_ylabel(r"Energy  (eV)",fontsize=20)
   ...
   cs = fig.colorbar(im, ax=ax,ticks=[0, 1 ])
   cs.ax.set_yticklabels([0, 1],fontsize=20)


   if len(fout) > 0:
      plt.savefig(fout+".png", bbox_inches='tight')
