...
def Fitting_Parameters(x1,nkx,nky,nkz,ψ,direccion,QQ,pos_fix):
    ...
    if direccion =='ky':
        ...
        poptz, pcovz = curve_fit( func_q_eq_0, x1,np.reshape(ψ[0,:], [nkx,nky,nkz])[pos_fix[0][0],pos_fix[1][0],:],p0=np.array([1,1,0]), bounds=((-np.inf,-np.inf,-np.inf), (np.inf,np.inf,np.inf)) )
        poptx[2] = 0.0
        poptz[2] = 0.0

    if direccion =='kz':
        poptx, pcovx = curve_fit( func_q_nq_0, x1,np.reshape(ψ[0,:], [nkx,nky,nkz])[:,pos_fix[1][0],pos_fix[2][0]],p0=np.array([1,1,-QQ,-np.sign(QQ)]), bounds=((-np.inf,-np.inf,-np.inf,-np.inf), (np.inf,np.inf,np.inf,np.inf)), method = 'trf' )
